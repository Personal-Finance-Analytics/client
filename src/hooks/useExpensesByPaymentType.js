export default function useExpensesByPaymentType(expensesData) {
  // Get array of unique expense type and amount in alphabetical order
  const unique = (value, index, self) => self.indexOf(value) === index;
  const categorisedData = expensesData.map((exp) => [exp.payment_type, exp.amount_nzd]).filter(unique).sort();
  ...
  const finalLabel = 'Expenses By Payment Type';
