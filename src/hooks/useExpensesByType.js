export default function useExpensesByType(expensesData) {
  // Get array of unique expense type and amount in alphabetical order
  const unique = (value, index, self) => self.indexOf(value) === index;
  const categorisedData = expensesData.map((exp) => [exp.type, exp.amount_nzd]).filter(unique).sort();
  ...
  categorisedData.forEach((exp) => {
    ...
    // Else if exp type already exists, simply add amount
  ...
  // console.table(accumulatedData);

  // Defineing final data attributes
  const finalLabel = 'Expenses By Type';
